...
/**
 * ================================================================
 * שחזור סיסמה ידני — למקרה שמישהו שכח סיסמה ולא מצליח להיכנס.
 * ----------------------------------------------------------------
 * רק מי שיש לו גישה לחשבון הגוגל של העסק יכול להריץ את זה — זו ההגנה.
 * הפונקציה לא חשופה באינטרנט; היא רצה רק מתוך עורך Apps Script.
 *
 * איך משתמשים (כ-30 שניות):
 *   1. נכנסים ל-script.google.com בחשבון העסק, פותחים את shiaim-server.
 *   2. משנים למטה את שתי השורות: מי המשתמש ומה הסיסמה החדשה.
 *   3. שומרים (Ctrl+S).
 *   4. למעלה בוחרים מהתפריט את הפונקציה RESET_PASSWORD ולוחצים ▶ הפעלה.
 *   5. זהו — הסיסמה החדשה עובדת מיד מכל מכשיר.
 * ================================================================
 */
function RESET_PASSWORD() {
  var username = 'yakov';          // מי שכח? 'yakov' או 'aharon'
  var newPassword = 'שנה-אותי';    // הסיסמה החדשה — לפחות 4 תווים

  // ---- אין צורך לגעת מכאן והלאה ----
  var u = findUser_(username);
  if (!u) throw new Error('משתמש לא נמצא: ' + username + ' (צריך yakov או aharon)');
  if (String(newPassword).length < 4) throw new Error('הסיסמה קצרה מדי — לפחות 4 תווים');
  var salt = randomToken_().slice(0, 16);
  var sh = getSheet_(SHEETS.users);
  sh.getRange(u.row, 4).setValue(salt);
  sh.getRange(u.row, 5).setValue(hashPassword_(salt, newPassword));
  Logger.log('✓ הסיסמה של ' + username + ' עודכנה בהצלחה. אפשר להיכנס עכשיו עם הסיסמה החדשה.');
}
